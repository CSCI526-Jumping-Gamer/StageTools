...
df = pd.read_csv("/Users/utkarshbhardwaj/Documents/Lectures/mobileDevices/analytics/new.csv", usecols = ['TOOLSPECIFICTYPE', 'SCENENAME'])
df['SCENENAME'] = df['SCENENAME'].fillna("NA")
df['TOOLSPECIFICTYPE'] = df['TOOLSPECIFICTYPE'].fillna("NA")

result = {}

toolSort = ['Accelerator', 'Magnet', 'Slingshot', 'Railgun', 'Rope', 'Rope with Magnet', 'Rotating Magnet']
toolSort.sort()
...
for ind in df.index:
    if df['SCENENAME'][ind] == "NA" or df['TOOLSPECIFICTYPE'][ind] == "NA":
        continue
    lvl = ""
...
plt.suptitle("Tools type used at each level", fontsize=14)
plt.show()
