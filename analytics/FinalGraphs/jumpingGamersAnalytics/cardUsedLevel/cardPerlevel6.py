...
for ind in df.index:
    if df['SCENENAME'][ind] == "NA" or df['EVENTNAME'][ind] not in ['playerDied', 'levelCompleted'] :
        continue
    if 'Level 6' in df['SCENENAME'][ind] and df['EVENTNAME'][ind] in ['playerDied', 'levelCompleted']:
        total_attempts +=1
    else:
        continue
...
for ind in df.index:
    if df['SCENENAME'][ind] == "NA" or df['CARDNAME'][ind] == "NA" :
        continue
    lvl = ""
    if 'Level 6' in df['SCENENAME'][ind]:
        crd = df['CARDNAME'][ind]
        if crd in result:
            result[crd] +=1
        else:
...
print(mylabels)
plt.pie(y, autopct=lambda p: '{:.0f}'.format(p * total / 100),startangle=90)
s = 'Card Used at level 6(total_Attempts = ' + str(total_attempts) + ')'
plt.title(s, fontsize=14)
plt.legend(labels=mylabels, loc='upper center',
           bbox_to_anchor=(0.5, -0.001), ncol=3)
plt.show()
