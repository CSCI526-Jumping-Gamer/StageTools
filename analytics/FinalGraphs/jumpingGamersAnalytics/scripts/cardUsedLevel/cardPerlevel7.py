...
for ind in df.index:
    if df['SCENENAME'][ind] == "NA" or df['EVENTNAME'][ind] not in ['playerDied', 'levelCompleted'] :
        continue
    if 'Level 7' in df['SCENENAME'][ind] and df['EVENTNAME'][ind] in ['playerDied', 'levelCompleted']:
        total_attempts +=1
    else:
        continue
...
for ind in df.index:
    if df['SCENENAME'][ind] == "NA" or df['CARDNAME'][ind] == "NA" :
        continue
    lvl = ""
    if 'Level 7' in df['SCENENAME'][ind]:
        crd = df['CARDNAME'][ind]
        if crd in result:
            result[crd] +=1
        else:
...
s = 'Card Used at level 7(total_Attempts = ' + str(total_attempts) + ')'
