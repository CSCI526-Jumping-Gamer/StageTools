...
df = pd.read_csv("/Users/utkarshbhardwaj/Documents/Lectures/mobileDevices/analytics/new.csv", usecols = ['TOOLNAME', 'SCENENAME'])
df['SCENENAME'] = df['SCENENAME'].fillna("NA")
df['TOOLNAME'] = df['TOOLNAME'].fillna("NA")

result = {}

toolSort = ['MagnetSlingshot', 'Magnet', 'AcceleratingTrail', 'RotatingMagnet', 'SmallMagnet', 'RopewithMagnet', 'RopeSegment', 'Railgun', 'childaccerlator' , 'RevisedRailgun', 'MediumMagnet']
...
for ind in df.index:
    if df['SCENENAME'][ind] == "NA" or df['TOOLNAME'][ind] == "NA":
        continue
    lvl = ""
...
New_Colors = ['green', 'blue', 'purple', 'brown', 'teal', 'red','yellow', 'black', 'pink', 'brown', 'orange']
